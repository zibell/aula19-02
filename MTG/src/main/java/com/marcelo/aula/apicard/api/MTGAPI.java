...
public abstract class MTGAPI {
	...
	protected static <TYPE> TYPE getByName(String path, String key,
			Class<TYPE> expectedClass) {
		Gson deserializer = new GsonBuilder().create();
		JsonObject jsonObject = getJsonObject(path, deserializer).get(0);
		try {
		
			JsonArray a = jsonObject.get("cards").getAsJsonArray();
			System.out.println(a.get(0));
			
			return deserializer.fromJson(a.get(0), expectedClass);
		}catch (Exception e) {
			return null;
		}
	}
